...
% legendCell = cellstr(strcat('\',num2str(eps_gaps', 'epsilon_g=%.2f')));
% legendCell{11} = 'FY 2010';
% save('fullmodel_eps_gamma_loop.mat')

load('fullmodel_eps_gamma_loop.mat')
load('colorblind_colormap.mat');
colors = colorblind([1 2 4:10 12],:,:);
figure(1); clf;
h = semilogx(mu_f,wavelns,'LineWidth',4.0,'Marker','o','Markersize',10); hold on;
...
figure(2); clf;
plot(eps_gaps,eps_props,'LineWidth',4.0,'Marker','o','Markersize',10); hold on;
p = polyfit(eps_gaps(1:5),eps_props(1:5),1); 
f = polyval(p,eps_gaps(1:7)); 
plot(eps_gaps(1:7),f,':','LineWidth',3.0); 
